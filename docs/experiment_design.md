...
4. **Assistant turns** are generated by an LLM with a **neutral system
   prompt** — concise, natural responses without metacognitive framing
   or experimental metadata. The assistant does not know about the
   benchmark question, the experiment design, or the compression
   conditions.

```
System prompt (assistant, dialogue generation):
"You are a helpful assistant. Keep responses concise and natural.
Do not make final decisions unless asked."

User turns (scripted, per sample):
