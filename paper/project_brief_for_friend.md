...
> 在固定压缩预算下，保留最近轮次原文是否总是有利于多轮推理？

我们的核心发现是：**不总是**。保留最近轮次能保护最近出现的关键证据，但如果真正有用的证据在较早轮次，它会挤占旧历史摘要预算，反而更容易丢失早期证据。
...
> 在相同压缩预算、相同压缩 prompt 下，最近轮次原文保留是提升多轮推理，还是造成位置依赖的取舍？

更具体地说：

- 当关键证据在最近窗口内，hybrid 是否更好？
- 当关键证据在较早历史中，hybrid 是否会更差？
...
| `one_shot_summary` | 全部历史一次性压缩 | 全局摘要 baseline |
| `hybrid_summary_recent` | 旧历史摘要 + 最近一轮原文 | 常见 recency-biased memory 策略 |
...
> hybrid 的效果不是小，而是方向相反的两个效应被平均掉了。关键证据最近时它明显更好；关键证据较早时它明显更差。
