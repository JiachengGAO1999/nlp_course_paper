...
- Data: MuSiQue QA items (Layer 1) + LLM-generated multi-turn dialogue; LongBench v2 (Layer 2 validation).
  ...
  evidence is incrementally introduced across user turns. The assistant uses a
  lightweight acknowledgement style (confirms receipt, preserves user-stated status
  in plain language, does not restructure or infer relations).
  ...
  The dialogue is generated by an LLM with a lightweight acknowledgement prompt
  — it embeds benchmark evidence into multi-turn conversations without turning
  the assistant into a structured note organizer.
...
- **Assistant**: LLM with lightweight acknowledgement prompt — confirms receipt,
  preserves user-stated status distinctions (source-backed / tentative / stale /
  background) in plain language, but does not restructure evidence, infer new
  relations, classify with formal labels, or repeat long evidence spans.
...
- 2026-06-13: **Assistant prompt update.** Changed from "structured note
  organizer" (separate hard facts, classify status) to "lightweight
  acknowledgement" — confirms receipt, preserves user-stated status in
  plain language, does not restructure evidence or infer relations. Avoids
  assistant pre-processing as a confound in compression comparisons. The older
  structured-output data should be treated as a future ablation candidate, not
  the main Layer 1 input.

## Next Steps

1. Regenerate smoke dialogues with the lightweight-acknowledgement assistant and
   inspect the preview for naturalness, no answer leakage, and no structured-note
   formatting.
...
lightweight-acknowledgement dialogue + 3 architecture conditions.
