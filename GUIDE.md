...
1. Write dialogue generation script: neutral-prompt LLM assistant + scripted
   user turns that inject benchmark evidence at varied positions.
2. Smoke test on 2–4 samples: generate dialogues → compress → validate → infer.
3. Pilot on 10–12 samples: all 3 conditions.
4. If pilot passes, freeze config → 40-sample formal run.
